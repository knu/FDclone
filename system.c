...
#ifdef	SYSV
static int NEAR Xgetdtablesize __P_((VOID_A));
#else
#define	Xgetdtablesize	getdtablesize
#endif
...
#endif
static int NEAR setshfunc __P_((char *, syntaxtree *));
static int NEAR unsetshfunc __P_((char *, int));
...
#define	constequal(s, c, l) \
			((l) == sizeof(c) - 1 && !strnenvcmp(s, c, l))
#define	consttail(s, h, t, l) \
			((l) == sizeof(h) - 1 + sizeof(t) - 1 \
...
#  ifndef	_NOKANJIFCONV
	...
	"FD_HEXPATH", "FD_CAPPATH", "FD_UTF8PATH", "FD_NOCONVPATH",
#  endif
# endif	/* FD */
};
...
FILE *fp;
{
	...
	fputs(long2str(buf, n, sizeof(buf)), fp);
}
...
FILE *fp;
{
	...
	if (signallist[i].sig < 0) {
		...
		ascnumeric(buf, (off_t)sig, i, width);
		fputs(buf, fp);
	}
...
int fd;
{
	...
	while ((n = Xread(fd, (char *)&ch, sizeof(ch))) < 0 && errno == EINTR);
	if (n < 0) return(-1);
	if (!n) return(READ_EOF);
	if (ch == '\r') {
		while ((n = Xread(fd, (char *)&ch, sizeof(ch))) < 0
		&& errno == EINTR);
		if (n < 0) return(-1);
		if (!n) {
			savech = READ_EOF;
...
#ifdef	SYSV
static int NEAR Xgetdtablesize(VOID_A)
{
# ifdef	USERESOURCEH
	struct rlimit lim;
...
int fdin, fdout;
{
	...
	for (;;) {
		while ((n = Xread(fdin, (char *)&ch, sizeof(ch))) < 0
		&& errno == EINTR);
		if (n <= 0) break;
		if (Xwrite(fdout, (char *)&ch, sizeof(ch)) < 0) return(-1);
	}
	return(n);
}
...
int len;
{
	...
	for (i = 0; var[i]; i++)
	if (!strnenvcmp(s, var[i], len) && var[i][len] == '=') break;
...
int len;
{
	...
	if (!strnenvcmp(s, ronlylist[i], len) && !ronlylist[i][len]) {
...
int len;
{
	...
	for (i = 0; exportlist[i]; i++)
		if (!strnenvcmp(s, exportlist[i], len) && !exportlist[i][len])
			break;
	if (!exportlist[i]) {
		exportlist = (char **)realloc2(exportlist,
			(i + 2) * sizeof(char *));
		exportlist[i] = strndup2(s, len);
		exportlist[++i] = NULL;
	}
...
# endif
	...
	for (i = 0; exportlist[i]; i++)
	if (!strnenvcmp(ident, exportlist[i], len) && !exportlist[i][len]) {
		free(exportlist[i]);
...
#ifndef	MINIMUMSHELL
#define	LINENOSIZ	(sizeof("LINENO") - 1)
...
long n;
{
	...
	shlineno = n;
	memcpy(tmp, "LINENO", LINENOSIZ);
	tmp[LINENOSIZ] = '=';
	long2str(&(tmp[LINENOSIZ + 1]), n, sizeof(tmp));
...
static VOID NEAR disphash(VOID_A)
{
	...
	if (hashtable) for (i = 0; i < MAXHASH; i++)
		for (hp = hashtable[i]; hp; hp = hp -> next) {
			long2str(buf, hp -> hits, sizeof(buf));
			j = strlen(buf);
			...
			fputs(buf, stdout);
			fputc(' ', stdout);
			ascnumeric(buf, (off_t)(hp -> cost), -1, 7);
			fputs(buf, stdout);
			fputc(' ', stdout);
...
syntaxtree *trp;
{
	...
	for (n = 1; n < (trp -> comm) -> argc; n++) {
		...
		if (!exportlist[i]) {
			exportlist = (char **)realloc2(exportlist,
				(i + 2) * sizeof(char *));
			exportlist[i] = strndup2(argv[n], len);
			exportlist[++i] = NULL;
		}
	...
	for (n = 1; n < (trp -> comm) -> argc; n++) {
		...
		if (!ronlylist[i]) {
			ronlylist = (char **)realloc2(ronlylist,
				(i + 2) * sizeof(char *));
			ronlylist[i] = strndup2(argv[n], len);
			ronlylist[++i] = NULL;
		}
	}
	return(ret);
...
static int NEAR setshfunc(ident, trp)
char *ident;
syntaxtree *trp;
{
	syntaxtree *functr;
...
#ifdef	MINIMUMSHELL
	functr = duplstree(functr, NULL);
#else
	functr = duplstree(functr, NULL, functr -> lineno);
#endif
	functr -> flags |= ST_TOP;
...
static int NEAR unsetshfunc(ident, len)
char *ident;
int len;
{
	int i;
...
# endif
	...
	else if (next) ret = setshfunc((trp -> comm) -> argv[0], next);
	else for (;;) {
		cont = 0;
#if	MSDOS
...
# endif
	...
	if (loginshell) {
		...
		execruncom(LSH_RUNCOMFILE, verbose);
	}
#endif	/* !FD */
#ifndef	MINIMUMSHELL
# if	!MSDOS
...
# endif
	...
	setshlineno(1L);
# if	!MSDOS
	setenv2("PPID", long2str(buf, getppid(), sizeof(buf)), 0);
# endif
#endif
...
# endif
	...
	if (loginshell && chdir3(getconstvar("HOME")) < 0) {
		fputs("No directory\n", stderr);
		fflush(stderr);
		Xexit2(RET_FAIL);
	}
