...
static VOID NEAR unset __P_((char *, int));
...
#endif
static int NEAR mkpipefile __P_((char *));
static int NEAR rmpipefile __P_((char *));
...
static int NEAR checktype __P_((char *, int *, int));
...
static int NEAR dochild __P_((syntaxtree *, int));
...
static char getflags[] = "xnvtsierkuhfam";
static char setflags[] = "xnvt\0\0e\0kuhfam";
static int *setvals[] = {
	&verboseexec,
	&notexec,
	&verboseinput,
...
#if	!MSDOS
# ifdef	NOJOB
	while (Xwait3(NULL, WNOHANG | WUNTRACED, NULL) > 0);
# else
	checkjob(1);
	stopped = 0;
# endif
...
# endif
	...
	tmp = (pid < 0) ? Xwait3(&w, opt, NULL) : Xwait4(pid, &w, opt, NULL);
...
# ifndef	NOJOB
	...
	if (!jobok) {
		...
		if (childpgrp < 0) childpgrp = (parent >= 0) ? parent : child;
...
{
	if (fd != STDIN_FILENO &&
	fd != STDOUT_FILENO && fd != STDERR_FILENO) Xclose(fd);
}
...
FILE *fp;
{
	...
	fd = fileno(fp);
	if (fd != STDIN_FILENO &&
	fd != STDOUT_FILENO && fd != STDERR_FILENO) Xfclose(fp);
}
...
redirectlist *redp;
{
	...
	if (redp -> new != redp -> fd) {
		Xclose(redp -> fd);
		if (redp -> new >= 0 && Xdup2(redp -> new, redp -> fd) < 0)
			return(redp);
	}
...
int len;
{
	...
	if (var[i]) {
		if (export) exportsize -= strlen(var[i]) + 1;
		free(var[i]);
		if (!s[len]) {
...
static VOID NEAR unset(ident, len)
char *ident;
int len;
{
	int i;
...
static int NEAR mkpipefile(buf)
char *buf;
{
	char *cp, path[MAXPATHLEN];
	int i, fd;
...
static int NEAR rmpipefile(path)
char *path;
{
	char *cp;
	int ret;
...
int fd;
{
	...
	if (pl -> file) {
		if (rmpipefile(pl -> file) < 0) doperror(NULL, pl -> file);
		ret = pl -> ret;
	}
#if	!MSDOS
	else if (!(pl -> pid)) {
...
/*ARGSUSED*/
static int NEAR checktype(s, idp, alias)
char *s;
int *idp, alias;
{
	int i;
...
static int NEAR dochild(trp, type)
syntaxtree *trp;
int type;
{
#if	MSDOS
	...
	if ((pid = makechild(1, -1)) < 0) return(-1);
		...
		if (!type && (trp -> flags & ST_TYPE) == OP_BG) return(ret);
...
syntaxtree *trp;
{
	...
	if ((trp -> comm) -> id != SM_IN) {
		...
		tmpargv = (char **)malloc((comm -> argc + 1) * sizeof(char *));
...
syntaxtree *trp;
{
	...
	if (argc >= 2) {
		if (restricted) {
			execerror(NULL, argv[1], ER_RESTRICTED);
			return(RET_FAIL);
		}
		argc--;
		free(argv[0]);
		for (i = 0; i <= argc; i++) argv[i] = argv[i + 1];
		path = evalexternal(trp -> comm);
#ifdef	BASHSTYLE
	/* bash ignores the unexecutable external command */
		if (!path) return(RET_NOTFOUND);
#endif
		path = strdup2(path);
		evar = exportvar;
		exportvar = NULL;
...
syntaxtree *trp;
{
	...
	for (n = 1; n < (trp -> comm) -> argc; n++) {
		len = strlen(argv[n]);
		if (checkprimal(argv[n], len) < 0
		|| checkronly(argv[n], len) < 0) {
			ret = RET_FAIL;
			ERRBREAK;
		}
		if (getshellvar(argv[n], len)) unset(argv[n], len);
		else unsetfunc(argv[n], len);
	}
	return(ret);
}
...
syntaxtree *trp;
{
	...
	if (!jobok || mypid != orgpgrp) return(RET_SUCCESS);
	checkjob(0);
	for (i = 0; i < maxjobs; i++) {
		if (!(joblist[i].pids)) continue;
		j = joblist[i].npipe;
...
syntaxtree *trp;
{
	...
	checkjob(0);
	if ((i = getjob(s)) < 0) {
		execerror(NULL, (trp -> comm) -> argv[1], ER_NOSUCHJOB);
		return(RET_FAIL);
	}
	n = joblist[i].npipe;
...
#ifdef	FD
		else if (type == CT_FDORIGINAL)
			fputs(" is a FDclone builtin", stdout);
		else if (type == CT_FDINTERNAL)
			fputs(" is a FDclone internal", stdout);
#endif
#if	MSDOS
		else if (type == CT_LOGDRIVE)
			fputs(" is a chenge drive command", stdout);
#endif
...
#ifdef	BASHSTYLE
	/* bash's "test" does not allow arithmatic comparison with strings */
				*ptrp = argc;
				return(-3);
#else
				v1 = 0;
...
/*ARGSUSED*/
static int NEAR exec_command(trp, bg, pipeend)
syntaxtree *trp;
int bg, pipeend;
{
#if	!MSDOS
...
#endif
	command_t *comm;
	char *path, **argv, **subst, **svar, **evar;
	long esize;
	int i, ret, id, type, argc, nsubst, *len;
...
#ifndef	BASHSTYLE
	/* bash treats the substitution at the end of pipeline as effective */
	if (pipeend && type != CT_COMMAND) {
		freevar(subst);
		free(len);
		comm -> argc = argc;
		freevar(comm -> argv);
		comm -> argv = argv;
		return(dochild(trp, CT_COMMAND));
	}
	...
	if (type == CT_NONE) ret = ret_status;
	else if (type == CT_BUILTIN) {
		if (!shbuiltinlist[id].func) ret = RET_FAIL;
		else if (restricted
		&& (shbuiltinlist[id].flags & BT_RESTRICT)) {
			execerror(NULL,
				(trp -> comm) -> argv[0], ER_RESTRICTED);
			ret = RET_FAIL;
		}
		else ret = (*shbuiltinlist[id].func)(trp);
	}
#ifdef	FD
	else if (type == CT_FDORIGINAL)
		ret = execbuiltin(id,
			(trp -> comm) -> argc, (trp -> comm) -> argv);
	else if (type == CT_FDINTERNAL) {
		if (shellmode) ret = RET_FAIL;
		else ret = execinternal(id,
			(trp -> comm) -> argc, (trp -> comm) -> argv);
	}
#endif
#if	MSDOS
	else if (type == CT_LOGDRIVE) {
		if (setcurdrv(id, 1) >= 0) ret = RET_SUCCESS;
		else {
			execerror(NULL, argv[0], ER_INVALDRIVE);
			ret = RET_FAIL;
		}
	}
#endif
	...
	else if (!(path = evalexternal(comm))) ret = ret_status;
	else {
#if	MSDOS
		if ((ret = spawnve(P_WAIT, path, comm -> argv, exportvar)) < 0
		&& errno == ENOENT) {
			execerror(NULL, argv[0], ER_COMNOFOUND);
			ret = RET_NOTFOUND;
		}
#else
		if (bg || ((trp -> flags & ST_TYPE) == OP_BG))
			Xexecve(path, comm -> argv, exportvar, 1);
		else if ((pid = makechild(1, -1)) < 0) ret = -1;
		else if (!pid) Xexecve(path, comm -> argv, exportvar, 1);
...
#endif
	{
		...
		else if (htable) freehash(htable);
#endif
		freevar(shellvar);
		freevar(exportvar);
		shellvar = svar;
...
# endif
	...
	if (!(trp -> comm) || pipein > 0) {
		...
		&& (errp = doredirect((trp -> comm) -> redp))) {
			...
			else if (jobok && mypid == orgpgrp) {
				long pid2;

				if ((pid2 = makechild(1, mypid)) < 0) ret = -1;
				else if (!pid2) {
					stackjob(mypid, 0, trp);
					ret = exec_statement(trp);
					prepareexit();
...
#if	!MSDOS && !defined (NOJOB) && !defined (USEFAKEPIPE)
		else if (!pipein || (ttypgrp < 0 && !(trp -> next)))
			ret = exec_command(trp, 1, 0);
#endif
		else if (!(trp -> next) && (trp -> flags & ST_NEXT)
		&& (tmptr = trp -> parent)
		&& (tmptr -> flags & ST_TYPE) == OP_PIPE)
			ret = exec_command(trp, 0, 1);
		else ret = exec_command(trp, 0, 0);
		if ((trp -> comm) -> redp)
...
# ifndef	NOJOB
	if ((trp -> flags & ST_TYPE) != OP_PIPE && jobok && mypid == orgpgrp)
		childpgrp = -1;
# endif
#endif
	if (interrupted) return(RET_INTR);
